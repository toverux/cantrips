...
Prose is sparse, plain, and uses the glossary terms (from `/codebase-design`) without ceremony.
...
- **Settled-decision callout** (if applicable) — one line in an amber-tinted box.
...
Plain English, concise — but the architectural nouns and verbs come straight from `/codebase-design`.
