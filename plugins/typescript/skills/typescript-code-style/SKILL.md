...
description: Load before writing or editing any TypeScript or JavaScript file (.ts, .tsx, .js, .jsx), the moment a task will create or modify such code and before the first edit. Covers strictness, the type system, nullability, immutability, and the language's core style conventions. The trigger is writing; a session that only reads code leaves it unloaded.
version: 3.0.0
---
...
A formatter owns mechanical layout — quote style, semicolons, trailing commas, arrow parentheses, import order — so match the project's formatter and leave those to it.
