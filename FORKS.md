...
- One dispatch is tiered: the `medium` finder carrying the mechanical lenses runs on the harness's mid-tier model, under `/simplify`'s carried model-selection paragraph with only its noun swapped; every other dispatch inherits the parent model, a `high` pass untiered because its bias is recall and a finder's misses reach no verifier.
