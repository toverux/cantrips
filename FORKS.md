...
How this plugin relates to every skill of its two upstreams — [mattpocock/skills](https://github.com/mattpocock/skills) at v1.2.3 and [EveryInc/compound-engineering-plugin](https://github.com/EveryInc/compound-engineering-plugin) at compound-engineering-v3.27.0.
...
- Renamed to this repo's skill name, with cross-skill references rewritten to this repo's names and the sidecar's `display_name` titlecasing the repo's name where upstream's differed.
...
- Every sidecar (`agents/openai.yaml`) states `policy.allow_implicit_invocation` explicitly, per AGENTS.md rule 2 — upstream ships sidecars of its own but leaves the policy block off its model-invoked skills — and the three CE forks carry a sidecar upstream lacks entirely.
- A closing flow pointer naming the next pipeline step(s), where the fork hands on to one, carrying the format clause from [flow-pointers.md](skills/writing-for-agents/flow-pointers.md) word for word and declaring several bulleted steps a chain or a choice.
  A closing that can point into `/simplify` or `/review-gate` also carries that file's compact hint — a full loop pass outgrows one context window, the agent cannot observe its own context level, and the user was compacting by hand before exactly those two steps.
...
- Every sub-agent dispatch asks for a background dispatch where the harness supports one and names the Claude Code parameter that gets it wrong (`run_in_background: false`) — no upstream says this, and a blocking dispatch freezes the session for as long as the agent runs.
  The parameter is named because the degradation is invisible from inside the skill and no harness-neutral wording reaches it; the model-selection paragraph `/simplify` carries from compound-engineering is the only other harness-specific setting a skill body names.

## mattpocock/skills

### /codebase-design (codebase-design)

Verified against `v1.2.3`.

- DESIGN-IT-TWICE.md step 2 opens "Spawn 3+ sub-agents, in parallel where the harness supports it (sequentially otherwise)" instead of upstream's unqualified "in parallel" — this plugin ships to Codex CLI as well, where fan-out may have to be sequential.
...
- The review tail adds a `/review-gate` severity suggestion (low for a trivial or mechanical diff, high for a large or risky one, medium otherwise) — it keeps the chain runnable in one session with the working diff as context, which renaming upstream's `/code-review` pointer alone would not.
...
- Upstream's two fixed axes (Standards + Spec, two parallel sub-agents, reports aggregated verbatim) replaced wholesale by an effort-scaled finder/verifier pipeline — `low`/`medium`/`high`, correctness angles A–F, quality lenses, independent verifiers returning CONFIRMED/PLAUSIBLE/REFUTED, a `high`-only sweep, a ranked capped report with outcome tracking, `--fix` apply mode, and a no-sub-agent fallback — because independent verification of every candidate is what buys precision at `medium` and recall at `high`, which unverified side-by-side axis reports cannot.
- Findings are deduped, merged by root cause, ranked, and capped, reversing upstream's explicit "do not merge or rerank findings" and dropping its _Why two axes_ rationale section — the separation upstream protected with that rule is served here by finder isolation, so ranking no longer lets one axis mask another; the rank order itself is stated — class, then verdict, then severity — since `--loop` orders its batches by it.
...
- That limit is a section of its own, "The mutation boundary": the run's target plus the import/export lines it needs, narrowed to whatever files the arguments named, with a fix that cannot stay inside it handed back rather than granted a wider scope.
...
- Each restraint is stated under the lens it qualifies and is self-contained there, and no lens names a phase of the skill that dispatched it, since a carrier is handed one lens section and both a pointer across to another lens and a reference to a caller's step resolve to nothing in its context.
  Simplification's consolidation restraint names the Reuse lens without pointing into it: the platform-guarantee route out of a duplicate is Reuse's call, and the restraint forbids proposing it from Simplification, which a carrier handed that section alone can obey whole.
  The route still gets proposed, since wherever Simplification is carried Reuse runs beside it — in the same finder, a parallel finder or fixer, or the one context of an inline fallback.
...
- A `--loop` flag added, implying `--fix` on every path — `low` and the no-sub-agent fallback included — and driving the gate to a defined green state, with its rules disclosed to [LOOP.md](skills/review-gate/LOOP.md) and loaded only when the flag is passed: fix batches verified by the project's own checks, self-scaling delta rounds between them, green declared only by a certifying pass at the invoked level that finds nothing new in the final tree, park-and-continue escalation of everything needing the user, fix-not-taking, repeat-question and novelty guards, and a closing report carrying the round and disposition ledgers.
...
- Apply mode, fork-added above, writes each fix from the line the finding quotes rather than its summary, rereads any prose it wrote before reporting the outcome, and applies in the report's rank order — a fix is a claim, and one written from a finding's summary was what the next round kept catching.
- One finder dispatch is tiered: the `medium` finder carrying the mechanical lenses runs on the harness's mid-tier model, under `/simplify`'s carried model-selection paragraph with only its noun swapped; every other finder, verifier and sweep dispatch inherits the parent model, a `high` pass untiered because its bias is recall and a finder's misses reach no verifier.
  Upstream dispatches every sub-agent on the parent model; the tier is narrower than compound-engineering's `ce-code-review` (`references/dispatch-reviewers.md` at `compound-engineering-v3.24.0`), which exempts only its correctness, security and adversarial personas and would tier the judgement lenses too.
- An **arbiter** added, its rules disclosed to [ARBITER.md](skills/review-gate/ARBITER.md) and loaded only by a run that applies fixes after a verify pass: one sub-agent on the harness's most capable model, dispatched alongside the verifiers, whose `fix` or `decline` on each finding binds — a decline is `skipped` with its reason and never put to the user — and whose opinion on how to fix is advisory; under `--loop` it is one agent messaged every round with the target's measured growth, and may have an overbuilt fix shrunk or backed out, once per fix ([ADR 0005](docs/adr/0005-an-authoritative-arbiter-of-proportionality-in-the-review-gate.md)).
  Upstream has no apply mode and so nothing to hold proportionate; here every loop round was locally reasonable and a small script still grew fivefold, which no stateless verifier and no standing instruction caught.
  Its model-selection paragraph is `/simplify`'s carried one with the tier raised and the Claude Code class sentence dropped.
- The sidecar's `short_description` is rewritten ("Effort-scaled, verified review of the changes since a fixed point") — upstream's "Review a diff on standards and spec" describes the two-axis design this fork replaced wholesale.
...
- The publish step's "Do NOT close or modify any parent issue." says "the parent spec" — this pipeline's parent artifact is the `/spec` document, not a tracker issue.
...
- "Pipeline closings" section added, sending the author to the sibling [flow-pointers.md](skills/writing-for-agents/flow-pointers.md) — the pipeline skills here all end on a flow pointer, and that file is the one home the format clause and compact hint are copied from.
  The copies ride in each closing because a pointer to the file failed twice: read as a citation under its `([presentation](…))` wording, then skipped under the verb-first "read … for the format", the agent inventing a closing format both times.
...
Verified against `compound-engineering-v3.27.0`.
...
- Upstream's "**Done when:** / **Stop when:**" header and its "Do not use `git diff HEAD` alone as cleanliness (it misses untracked files)" and "never compare against `origin/<name>`" guards not carried — each states what the fork already does elsewhere.
...
- Upstream's `exclude:<paths>` invocation contract not carried — no caller here passes one, `/commit` being invoked bare — and Step 5's staged-index rationale drops upstream's "a caller's `exclude:` paths" example with it, the rest of the passage riding byte-identical.
- Upstream's plan-unit subject trailer (append the unit's U-ID, `(U3)`, when a plan Implementation Unit is in hand) not carried — it is ce-plan's plan-format vocabulary, which has no counterpart here; `/spec` is this pipeline's plan artifact and defines no unit IDs.
- Step 5 adds two guard lines after upstream's path-list rationale — during a merge or rebase drop the path list (git refuses a partial commit) and check the index first, the bare commit taking it whole; and commit straight after staging, since the path list takes the named files' working-tree content — edges upstream's own text leaves undocumented.
...
- Upstream's worth lens (added by v3.27.0: a `## Worth lens` section that reads a clean-up intent off the arguments and confirms it, `references/worth-audit.md`, and a recommend-only limb in `references/modes.md`) reduces to one judgment rule, "Redundant needs a quoted artifact", carrying its positive-evidence test and its partly-recoverable route to Update in upstream's phrases, so a future sync reconciles that rule against `references/worth-audit.md`.
  The opt-in confirmation guards accurate docs against an unattended delete, where every Delete here already reaches the user's gate one by one, and the bar the lens applies is a block shared with `ce-compound`, `/compound` stating its own.
...
- Upstream's blocking-question protocol not carried — host-neutral capability matching, tool-discovery loading, a numbered-options fallback, and ask-one-at-a-time / recommended-option-first principles, in `references/modes.md` as of v3.24.0 — the fork presents its verdicts for approval together, in plain prose.
...
- Upstream's run-word rule (added to `references/scope.md` by v3.27.0: an operation word, a worth-lens intent or a mode token is removed before the argument is read as a scope hint) not carried — the last two name machinery the fork lacks, and an operation word that matches nothing already lands on the fork's visible fallback, "say so in one line and audit `AGENTS.md` alone".
...
- Body rewritten; the three paragraphs merged from upstream stay byte-identical: model selection, which `/review-gate` also carries with its noun swapped, the mutation boundary, and pre-release compatibility scaffolding.
...
- "Bounded dispatch" paragraph carried in part: its queueing and active-agent-limit backpressure is dropped — the harness's business, and three fixed reviewers never reach the limit — while its inline-fallback rule stays in Step 2, a lost dispatch otherwise costing the pass a whole lens in silence.
  That rule is reworded rather than carried: the fork adds the harness-with-no-dispatch-primitive branch — the case upstream's own "run the reviews inline or serially" covered — and keeps upstream's v3.24.0 test, "for a reason that survives correcting the invocation", where v3.27.0 widens it to "cannot recover through active work, supported release, or a corrected invocation" — the first two routes belong to the queueing dropped above and to the agent-lifecycle paragraph skipped below, which leaves the corrected invocation as the only recovery a fixer dispatch has here; the rest of the sentence tracks v3.27.0.
...
- Upstream's "Agent lifecycle" paragraph (added by v3.27.0) not carried — collecting every outcome before cleanup, closing or releasing review-owned agents where the harness offers caller-owned cleanup, never messaging a finished agent or inferring freed capacity — harness housekeeping like the task-tracking paragraph, its slot-refilling clause serving the queueing dropped above; Step 3 already holds the pass until all three lens passes are complete.
...
- Upstream's harness tool mechanics not carried — Step 1's blocking-question protocol (host-neutral capability matching as of v3.24.0) and Step 2's subagent-primitive enumeration — the enumerated names go stale, and a tool-use protocol is the harness's business, not the skill's; the background-dispatch parameter in Step 2 is the standing exception the systematic conventions record.
...
Not ported. `/prototype` (forked from Pocock) is this pipeline's prototyping step; ce-prototype's contract and handoffs ride CE's outer loop (ce-brainstorm, ce-plan).
