...
2. **Shared root `plugin.json` per plugin — cheap triple play.** One additive file
   ...
   CLI's documented manifest location. Extend release-please's `extra-files` to cover its version.
   Caveat: per-plugin install from a multi-plugin repo is unverified for
...
every copy is kept in sync by hand, so weigh that before adding any of them.
