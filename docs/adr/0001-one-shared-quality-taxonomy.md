...
2026-09-01: three of this record's claims are corrected below; the decision — one shared lens file read by both skills — stands, so the status does not change.
The second correction is an instance of [ADR 0003](0003-byte-identity-on-carried-text.md), which holds the reasoning on carried text.

- "Design and Conventions moved across unchanged" is false against upstream. Design carries recorded divergences — `Mysterious Name` names a function, variable or type, `Primitive Obsession` covers a string, and the prose gloss above — and Conventions has no upstream counterpart: upstream ships a Standards sub-agent brief, and the lens was written here from it. `FORKS.md`'s `/review-gate` section records both.
- "Two passages previously held byte-identical to upstream are compressed" is half true. Upstream had already compressed its code-reuse rule before the pin, so that passage was not byte-identical when this fork compressed it further; the code-quality rule exists in full at the pin, and that one this fork genuinely compressed.
- "Every section of that file is self-contained, because both skills dispatch a carrier exactly one lens at a time" is false in both halves. The Simplification lens's consolidation restraint hands the platform-guarantee route out of a duplicate to the Reuse lens "under the conditions printed there", unreadable to a carrier holding Simplification alone; and `/review-gate` at `medium` hands one finder the three mechanical lenses together. `FORKS.md` records which carriers hit the dead end, and `IDEAS.md` holds the fix options.

2026-09-19: the dead end named in the third correction is closed. The restraint now hands the route to the Reuse lens without pointing into its text, so every section is self-contained; the route is still proposed, since Reuse runs wherever Simplification is carried. `FORKS.md` holds the record, and the `IDEAS.md` entry is gone.
