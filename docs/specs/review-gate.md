...
Supersedes the `code-review` and `grill-me` rows of the original v1 design.
...
- **Pure ranking at the report cap**: correctness (spec included) before quality, CONFIRMED before PLAUSIBLE, no per-class floor — the cap sizes one fix batch, and squeezed quality findings stay requestable in-session and get another chance on the post-fix re-run; a floor could displace verified bugs at `high`, where "a missed bug ships".
...
Verification is dogfooding: running `/review-gate` on this repo's own diffs is the acceptance test, and this spec is what its spec finder reviews future changes against.
...
- Renaming any other skill; updating the v1 design record.
