...
The principles reach past skills: an `AGENTS.md`, a rules file, or a `CLAUDE.md` is a skill body by another name — prose an agent loads and acts on.
Apply everything below when writing or editing those files too; only the Invocation section is skill-specific (those files load unconditionally, which makes the pruning discipline bite even harder).
...
- **One trigger per branch.**
  Synonyms that rename a single branch are **duplication** — "build features using TDD … asks for test-first development" is one branch written twice.
  Collapse them; keep only genuinely distinct branches.
- **Cut identity that's already in the body.**
  Keep the description to triggers, plus any "when another skill needs…" reach clause.
...
- **By sequence** — split a run of **steps** when the steps still ahead (a step's **post-completion steps**) tempt the agent to rush the one in front of it (**premature completion**).
  Keeping them out of view encourages the agent to do more **legwork** on the current task.
...
Then hunt **no-ops** sentence by sentence, not just line by line: run the no-op test on each sentence in isolation, and when one fails, delete the whole sentence rather than trim words from it.
...
A triad spelled out at three sites (**duplication**), a description spending a sentence to gesture at one idea — each is a passage begging to **collapse** into a single token.
Examples include:
...
- **Duplication** — the same meaning in more than one place.
  Costs maintenance and tokens, and inflates a meaning's prominence on the ladder past its real rank.
...
- **No-op** — a line the model already obeys by default, so you pay load to say nothing.
  ...
  So does rationale: a justification the reader would have followed the rule without is a no-op in a persuasive voice.
...
- **Director's commentary** — how the conclusion was reached, shipped beside the conclusion.
  The reader acts on the finding rather than on the search that found it, so the derivation is load bought for nothing.
  The cure is placement before deletion: move it where someone would go to redo the work, or drop it.
