...
Done when every criterion is verified, the ticket (if any) is resolved, and the full suite passes → close with a flow pointer ([presentation](../writing-great-skills/flow-pointers.md)) naming the review tail, all in this session with the working diff as context: `/simplify` (user-invoked) for an optional quality pass, then `/review-gate` (user-invoked) — suggest `low` for a trivial or mechanical diff, `high` for a large, cross-cutting, or risky one, `medium` otherwise — then `/commit` (user-invoked).
